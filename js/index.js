...
function handlePost(editMode) {
    ...
    var title = document.getElementById('post-content').value;
    var text = document.querySelector('input[name="description"]').value;
    var post_id = document.getElementById('post-id').value;

    // Make an AJAX request to the PHP script
    $.ajax({
        type: 'POST',
        url: 'db3.php', // Replace with the correct path to your PHP script
        data: {
            post_content: title,
            description: text,
            post_id: post_id,
        ...
        success: function (response) {
            ...
            if (editMode) {
                updatePost(post_id, title, text);
            } else {
                // Create a new div to display the post
                var postDiv = createPostDiv(title, text);
...
function getCurrentDateTime() {
    var now = new Date();
    var dateTimeString = now.toLocaleString(); // You can format the date and time as needed
    return dateTimeString;
}

// Function to create a new post div
function createPostDiv(title, text, post_id) {
    ...
    // Create elements for title, text, date, and time
    ...
    likeButton.className = 'btn btn-success like-btn';
    ...
    commentButton.className = 'btn btn-primary comment-btn';
...
function handleEdit(post_id, title, text) {

    // Set the form fields with the post content for editing
    document.getElementById('post-content').value = title;
    document.querySelector('input[name="description"]').value = text;

    // Set the edit mode and post ID
...
function updatePost(post_id, title, text) {
